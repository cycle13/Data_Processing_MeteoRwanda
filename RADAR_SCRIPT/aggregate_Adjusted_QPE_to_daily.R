...
############
## minimum number of available adjusted hourly QPE to aggregate.
...
dir.1hr.adj <- file.path(radar_data_dir, "Adjusted_QPE_1hr")
if(!dir.exists(dir.1hr.adj)) stop("The folder containing the adjusted 1hr QPE does not found\n")
dir.daily.adj <- file.path(radar_data_dir, "Adjusted_QPE_daily")
if(!dir.exists(dir.daily.adj)) dir.create(dir.daily.adj, showWarnings = FALSE, recursive = TRUE)
cat("Aggregate hourly adjusted QPE to daily .........\n")
...
cat("Aggregating hourly adjusted QPE to daily finished successfully\n")
